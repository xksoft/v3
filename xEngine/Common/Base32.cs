...
namespace xEngine.Common
{
    ...
    public class Base32
    {
        ...
        ///     White space is not permitted when decoding (not ignored).
        /// </summary>
        /// <param name="alternateAlphabet">Alphabet to use (such as Base32.ZBase32Alphabet)</param>
        ...
        ///     Alphabet to use (such as Base32.ZBase32Alphabet, Base32.Base32StandardAlphabet or your
        ///     own custom 32 character alphabet string)
        /// </param>
        ...
        public byte[] Decode(string input)
        {
            ...
            for (var i = 0; i < input.Length; i += 8)
            {
                ...
                for (var charOffset = 0; charOffset < chars; charOffset++)
                {
                    ...
                    {
                        throw new ArgumentException("Invalid character '" + input.Substring(i + charOffset, 1) +
            ...
            if (_index == null)
            {
                ...
                if (!Indexes.TryGetValue(indexKey, out cidx))
                {
                    lock (Indexes)
                    {
                        if (!Indexes.TryGetValue(indexKey, out cidx))
                        {
                            cidx = new Dictionary<string, uint>(_alphabet.Length,
                                IsCaseSensitive

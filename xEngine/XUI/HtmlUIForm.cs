...
using xEngine.XUI.ChromiumCore;
using xEngine.XUI.Internal;
...
namespace xEngine.XUI
{
	...
	public class HtmlUIForm : Form, IChromiumWebBrowser
	{
		...
		/// </summary>
		private const string JS_WINDOW_SYSTEM_COMMAND_NOTIFY = @"(function(){{document.body.addEventListener('click', function (e) { var region = e.srcElement; var cmd = region.className.includes('-nanui-close') ? 'close' : region.className.includes('-nanui-minimize') ? 'minimize' : region.className.includes('-nanui-maximize') ? 'maximize' : null;  if (cmd != null && XUI) { XUI.hostWindow[cmd].apply(); e.preventDefault(); region.blur(); } }, false);}})();";
		...
		protected void InitializeChromium(string initialUrl)
		{
			...
			GlobalObject.Add("XUI", new JsHostWindowObject(this));
		}

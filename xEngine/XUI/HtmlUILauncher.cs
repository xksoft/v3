...
using xEngine.XUI.ChromiumCore.Event;
using xEngine.XUI.Resource;
...
namespace xEngine.XUI
{
	...
	public class HtmlUILauncher
	{
        ...
        private static readonly string RuntimeDir = System.IO.Path.GetFullPath("./")+ "ceffx";
		...
		private static bool IsRuntimeExists()
		{
			...
			if (PlatformArch == RuntimeArch.x64)
				FrameworkDir = RuntimeDir+"\\x64";
			else
				FrameworkDir = RuntimeDir + "\\x86";

			LocalesDir = RuntimeDir+"\\Resources\\locales";
			ResourcesDir = RuntimeDir+ "\\Resources";

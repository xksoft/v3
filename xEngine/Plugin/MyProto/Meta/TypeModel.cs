...
namespace MyProto.Meta
{
    ...
    public abstract class TypeModel
    {
            ...
            SerializationContext context)
        {
            ...
            using (var reader = new ProtoReader(source, this, context, len))
            {
                var key = GetKey(ref type);
                ...
                {
                    if (
                        !(TryDeserializeAuxiliaryType(reader, DataFormat.Default, Serializer.ListItemTag, type,
                            ref value, true, false, true, false) || len == 0))
                    {
        ...
        private class DeserializeItemsIterator : IEnumerator, IEnumerable
        {
            ...
            public bool MoveNext()
            {
                ...
                {
                    int bytesRead;
                    Current = _model.DeserializeWithLengthPrefix(_source, null, _type, _style, _expectedField, _resolver,
                        out bytesRead, out _haveObject, _context);
                }
                return _haveObject;
            }
        ...
        public object Deserialize(Stream source, object value, Type type, SerializationContext context)
        {
            ...
            using (var reader = new ProtoReader(source, this, context))
            {
...
#else
            var autoCreate = PrepareDeserialize(value, ref type);
            using (var reader = new ProtoReader(source, this, context, length))
            {
                if (value != null) reader.SetRootObject(value);
                var obj = DeserializeCore(reader, type, value, autoCreate);
                reader.CheckFullyConsumed();
        ...
        internal static MethodInfo ResolveListAdd(TypeModel model, Type listType, Type itemType, out bool isList)
        {
            ...
            if (add == null)
            {
                ...
                var forceList = listTypeInfo.IsInterface &&
                                listTypeInfo == model.MapType(typeof (IEnumerable<>)).MakeGenericType(types)
#if WINRT
                    .GetTypeInfo()
#endif
                    ;
...
#endif
                    if (interfaceType.Name == "IProducerConsumerCollection`1" && interfaceType.IsGenericType &&
    ...
    // more convenient GetProperty overload not supported on all platforms
        ...
        public object DeepClone(object value)
        {
            ...
            if (key >= 0 && !Helpers.IsEnum(type))
            {
                using (var ms = new MemoryStream())
                {
                    ...
                    ms.Position = 0;
                    using (var reader = new ProtoReader(ms, this, null))
                    {
            ...
            if (GetWireType(Helpers.GetTypeCode(type), DataFormat.Default, ref type, out modelKey) != WireType.None &&
                modelKey < 0)
            {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                {
                    if (
                        !TrySerializeAuxiliaryType(writer, type, DataFormat.Default, Serializer.ListItemTag, value,
                            false)) ThrowUnexpectedType(type);
                    writer.Close();
                }
                ms.Position = 0;
                using (var reader = new ProtoReader(ms, this, null))
                {
                    ...
                    TryDeserializeAuxiliaryType(reader, DataFormat.Default, Serializer.ListItemTag, type, ref value,
        ...
        protected internal static void ThrowUnexpectedType(Type type)
        {
            ...
            if (type != null)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Are you mixing protobuf-net and protobuf-csharp-port");
                }
            }
#endif

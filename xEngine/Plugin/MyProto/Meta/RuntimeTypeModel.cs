...
namespace MyProto.Meta
{
    ...
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        /// <summary>
        ///     The default model, used to support MyProto.Serializer
        /// </summary>
        ...
        public override string GetSchema(Type type)
        {
            ...
            if (type == null)
            {
                ...
                foreach (MetaType meta in _types)
                {
            ...
            if (!isInbuiltType)
            {
                IEnumerable typesForNamespace = primaryType == null ? _types : requiredTypes;
        ...
        private void CascadeDependents(BasicList list, MetaType metaType)
        {
            ...
            if (metaType.IsList)
            {
                ...
                WireType defaultWireType;
                var coreSerializer = ValueMember.TryGetCoreSerializer(this, DataFormat.Default, itemType,
                ...
                if (metaType.IsAutoTuple)
                {
                    ...
                    if (MetaType.ResolveTupleConstructor(metaType.Type, out mapping) != null)
                    {
                        for (var i = 0; i < mapping.Length; i++)
                        {
                            ...
                            WireType defaultWireType;
                            var coreSerializer = ValueMember.TryGetCoreSerializer(this, DataFormat.Default,
                                type,
                                out defaultWireType, false, false, false, false);
                            if (coreSerializer == null)
                            {
        ...
        {
            get { return (MetaType) _types[FindOrAddAuto(type, true, false, false)]; }
        }
        ...
        private sealed class MetaTypeFinder : BasicList.IPredicate
        {
            ...
            {
                return ((MetaType) obj).Type == _type;
            }
        }
        ...
        private sealed class BasicTypeFinder : BasicList.IPredicate
        {
            ...
            public bool IsMatch(object obj)
            {
                return ((BasicType) obj).Type == _type;
            }
        }
        ...
        private readonly BasicList _basicTypes = new BasicList();
        ...
        internal IProtoSerializer TryGetBasicTypeSerializer(Type type)
        {
            ...
            var idx = _basicTypes.IndexOf(predicate);
            ...
            lock (_basicTypes)
            {
                ...
                // double-checked
                idx = _basicTypes.IndexOf(predicate);
                if (idx >= 0) return ((BasicType) _basicTypes[idx]).Serializer;
        ...
        {
            var predicate = new MetaTypeFinder(type);
            var key = _types.IndexOf(predicate);
            MetaType metaType;
            ...
            if (key < 0)
            {
                ...
                try
                {
                    ...
                    // double-checked
                    var winner = _types.IndexOf(predicate);
                    if (winner < 0)
                    {
                        ThrowIfFrozen();
                        key = _types.Add(metaType);
                        weAdded = true;
                    }
                    else
                    {
...
//#endif
        ...
        internal int GetKey(Type type, bool demand, bool getBaseKey)
        {
            ...
            try
            {
                ...
                if (typeIndex >= 0)
                {
                    var mt = (MetaType) _types[typeIndex];
                    if (getBaseKey)
                    {
        ...
        protected internal override void Serialize(int key, object value, ProtoWriter dest)
        {
            ...
            ((MetaType) _types[key]).Serializer.Write(value, dest);
#endif
        }
        ...
        protected internal override object Deserialize(int key, object value, ProtoReader source)
        {
            ...
            IProtoSerializer ser = ((MetaType) _types[key]).Serializer;
...
#if FEAT_COMPILER && !FX11
            if (compiled) return CompilerContext.BuildSerializer(serializer, this);
#endif
        ...
        internal sealed class SerializerPair : IComparable
        {
            ...
            public SerializerPair(int metaKey, int baseKey, MetaType type, MethodBuilder serialize,
            ...
            int IComparable.CompareTo(object obj)
            {
                ...
                if (BaseKey == MetaKey)
                {
                    if (other.BaseKey == other.MetaKey)
                    {
                        ...
                        return MetaKey.CompareTo(other.MetaKey);
                    }
                ...
                var result = BaseKey.CompareTo(other.BaseKey);
                if (result == 0) result = MetaKey.CompareTo(other.MetaKey);
                return result;
            }
        }
        ...
        {
            var baseMethod = type.BaseType.GetMethod(name, BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            var newMethod = type.DefineMethod(baseMethod.Name,
                (baseMethod.Attributes & ~MethodAttributes.Abstract) | MethodAttributes.Final,
                baseMethod.CallingConvention, baseMethod.ReturnType, paramTypes);
    ...
    /// </summary>
        ...
        public sealed class CompilerOptions
        {
            ...
            public void SetFrameworkOptions(MetaType from)
            {
                ...
                var attribs = AttributeMap.Create(from.Model, from.Type.Assembly);
        ...
        public TypeModel Compile(CompilerOptions options)
        {
            ...
            BuildAllSerializers();
            Freeze();
            var save = !Helpers.IsNullOrEmpty(path);
            ...
            {
                assemblyName = new FileInfo(Path.GetFileNameWithoutExtension(path)).Name;
                moduleName = assemblyName + Path.GetExtension(path);
            }
            ...
            an.Name = assemblyName;
            var asm = AppDomain.CurrentDomain.DefineDynamicAssembly(an,
                (save ? AssemblyBuilderAccess.RunAndSave : AssemblyBuilderAccess.Run)
                );
            var module = save
                ? asm.DefineDynamicModule(moduleName, path)
                : asm.DefineDynamicModule(moduleName);
#endif

            WriteAssemblyAttributes(options, assemblyName, asm);

            var type = WriteBasicTypeModel(options, typeName, module);
            ...
            WriteGetKeyImpl(type, hasInheritance, methodPairs, ilVersion, out il, out knownTypesCategory, out knownTypes,
                out knownTypesLookupType);

            var ctx = WriteSerializeDeserialize(assemblyName, type, methodPairs, ilVersion, ref il);

            WriteConstructors(type, ref index, methodPairs, ref il, knownTypesCategory, knownTypes, knownTypesLookupType,
                ctx);
        ...
        private void WriteConstructors(TypeBuilder type, ref int index, SerializerPair[] methodPairs, ref ILGenerator il,
            int knownTypesCategory, FieldBuilder knownTypes, Type knownTypesLookupType, CompilerContext ctx)
        {
            ...
            switch (knownTypesCategory)
            {
                ...
                {
                    CompilerContext.LoadValue(il, _types.Count);
                ...
                {
                    CompilerContext.LoadValue(il, _types.Count);
                    var loc = il.DeclareLocal(knownTypesLookupType);
                    il.Emit(OpCodes.Newobj, knownTypesLookupType.GetConstructor(new[] {MapType(typeof (int))}));
                    il.Emit(OpCodes.Stsfld, knownTypes);
                    var typeIndex = 0;
                    foreach (var pair in methodPairs)
                    {
                        ...
                        CompilerContext.LoadValue(il, keyIndex);
                        il.EmitCall(OpCodes.Callvirt,
                            knownTypesLookupType.GetMethod("Add", new[] {MapType(typeof (Type)), MapType(typeof (int))}),
                            null);
                    }
                    il.Emit(OpCodes.Ret);
                }
                ...
                case KnownTypesHashtable:
                {
                    ...
                    foreach (var pair in methodPairs)
                    {
                        ...
                        il.EmitCall(OpCodes.Callvirt,
                            knownTypesLookupType.GetMethod("Add",
                                new[] {MapType(typeof (object)), MapType(typeof (object))}), null);
                    }
                    il.Emit(OpCodes.Ret);
                }
                    break;
        ...
        private CompilerContext WriteSerializeDeserialize(string assemblyName, TypeBuilder type,
            SerializerPair[] methodPairs, CompilerContext.IlVersion ilVersion, ref ILGenerator il)
        {
            il = Override(type, "Serialize");
            var ctx = new CompilerContext(il, false, true, methodPairs, this, ilVersion, assemblyName,
                MapType(typeof (object)));
            // arg0 = this, arg1 = Key, arg2=obj, arg3=dest
            var jumpTable = new Label[_types.Count];
            ...
            ctx = new CompilerContext(il, false, false, methodPairs, this, ilVersion, assemblyName,
                MapType(typeof (object)));
            ...
            for (var i = 0; i < jumpTable.Length; i++)
            {
                var pair = methodPairs[i];
                il.MarkLabel(jumpTable[i]);
                var keyType = pair.Type.Type;
                if (keyType.IsValueType)
                {
                    il.Emit(OpCodes.Ldarg_2);
                    il.Emit(OpCodes.Ldarg_3);
                    il.EmitCall(OpCodes.Call,
                        EmitBoxedSerializer(type, i, keyType, methodPairs, this, ilVersion, assemblyName), null);
                    ctx.Return();
                }
                else
                {
        ...
        private const int KnownTypesArray = 1,
            KnownTypesDictionary = 2,
            KnownTypesHashtable = 3,
            KnownTypesArrayCutoff = 20;

        private void WriteGetKeyImpl(TypeBuilder type, bool hasInheritance, SerializerPair[] methodPairs,
            CompilerContext.IlVersion ilVersion, out ILGenerator il, out int knownTypesCategory,
            out FieldBuilder knownTypes, out Type knownTypesLookupType)
        {
            il = Override(type, "GetKeyImpl");

            if (_types.Count <= KnownTypesArrayCutoff)
            {
                knownTypesCategory = KnownTypesArray;
                knownTypesLookupType = MapType(typeof (Type[]), true);
            }
            else
            {
#if NO_GENERICS
...
#else
                knownTypesLookupType = MapType(typeof (Dictionary<Type, int>), false);
#endif
            ...
            knownTypes = type.DefineField("knownTypes", knownTypesLookupType,
            ...
            switch (knownTypesCategory)
            {
                case KnownTypesArray:
                {
                    ...
                    il.EmitCall(OpCodes.Callvirt, MapType(typeof (IList)).GetMethod(
                        "IndexOf", new[] {MapType(typeof (object))}), null);
                    if (hasInheritance)
                    {
                        ...
                        var getKeyLabels = new BasicList();
                        var lastKey = -1;
                        ...
                        var subtypeLabels = new Label[getKeyLabels.Count];
                        ...
                        for (var i = subtypeLabels.Length - 1; i >= 0; i--)
                        {
                            if (lastKey != methodPairs[i].BaseKey)
                            {
                                lastKey = methodPairs[i].BaseKey;
                                ...
                                // the base-type)
                                var keyIndex = -1;
                                for (var j = subtypeLabels.Length; j < methodPairs.Length; j++)
                                {
                                    if (methodPairs[j].BaseKey == lastKey && methodPairs[j].MetaKey == lastKey)
                                    {
                                        keyIndex = j;
                ...
                case KnownTypesDictionary:
                {
                    var result = il.DeclareLocal(MapType(typeof (int)));
                    var otherwise = il.DefineLabel();
                    il.Emit(OpCodes.Ldsfld, knownTypes);
                    il.Emit(OpCodes.Ldarg_1);
                    il.Emit(OpCodes.Ldloca_S, result);
                    il.EmitCall(OpCodes.Callvirt,
                        knownTypesLookupType.GetMethod("TryGetValue", BindingFlags.Instance | BindingFlags.Public), null);
        ...
        private void WriteSerializers(CompilerOptions options, string assemblyName, TypeBuilder type, out int index,
            out bool hasInheritance, out SerializerPair[] methodPairs, out CompilerContext.IlVersion ilVersion)
        {
            CompilerContext ctx;
            ...
            foreach (MetaType metaType in _types)
            {
                var writeMethod = type.DefineMethod("Write"
#if DEBUG
                                                              + metaType.Type.Name
#endif
                    ,
                    MethodAttributes.Private | MethodAttributes.Static, CallingConventions.Standard,
                    MapType(typeof (void)), new[] {metaType.Type, MapType(typeof (ProtoWriter))});

                var readMethod = type.DefineMethod("Read"
#if DEBUG
                                                             + metaType.Type.Name
#endif
                    ,
                    MethodAttributes.Private | MethodAttributes.Static, CallingConventions.Standard,
                    metaType.Type, new[] {metaType.Type, MapType(typeof (ProtoReader))});

                var pair = new SerializerPair(
            ...
            {
                ilVersion = CompilerContext.IlVersion.Net1; // old-school!
            }
            for (index = 0; index < methodPairs.Length; index++)
            {
                var pair = methodPairs[index];
                ctx = new CompilerContext(pair.SerializeBody, true, true, methodPairs, this, ilVersion, assemblyName,
                    pair.Type.Type);
                ctx.CheckAccessibility(pair.Deserialize.ReturnType);
                pair.Type.Serializer.EmitWrite(ctx, ctx.InputValue);
                ctx.Return();

                ctx = new CompilerContext(pair.DeserializeBody, true, false, methodPairs, this, ilVersion, assemblyName,
                    pair.Type.Type);
        ...
        {
            var baseType = MapType(typeof (TypeModel));
            var typeAttributes = (baseType.Attributes & ~TypeAttributes.Abstract) | TypeAttributes.Sealed;
            if (options.Accessibility == Accessibility.Internal)
            {
                typeAttributes &= ~TypeAttributes.Public;
            }

            var type = module.DefineType(typeName, typeAttributes, baseType);
            return type;
        }
        ...
        private void WriteAssemblyAttributes(CompilerOptions options, string assemblyName, AssemblyBuilder asm)
        {
            if (!Helpers.IsNullOrEmpty(options.TargetFrameworkName))
            {
                ...
                try
                {
                    ...
                    versionAttribType = GetType("System.Runtime.Versioning.TargetFrameworkAttribute",
                        MapType(typeof (string)).Assembly);
                }
                catch
                {
                    /* don't stress */
                ...
                if (versionAttribType != null)
                {
                    ...
                    var builder = new CustomAttributeBuilder(
                        versionAttribType.GetConstructor(new[] {MapType(typeof (string))}),
            ...
            try
            {
                internalsVisibleToAttribType = MapType(typeof (InternalsVisibleToAttribute));
            }
            catch
            {
                /* best endeavors only */
            ...
            if (internalsVisibleToAttribType != null)
            {
                ...
                foreach (MetaType metaType in _types)
                {
                    var assembly = metaType.Type.Assembly;
                    if (consideredAssemblies.IndexOfReference(assembly) >= 0) continue;
                    consideredAssemblies.Add(assembly);

                    var assemblyAttribsMap = AttributeMap.Create(this, assembly);
                    for (var i = 0; i < assemblyAttribsMap.Length; i++)
                    {
                        ...
                        var privelegedAssemblyName = privelegedAssemblyObj as string;
                        if (privelegedAssemblyName == assemblyName || Helpers.IsNullOrEmpty(privelegedAssemblyName))
                            continue; // ignore

                        if (internalAssemblies.IndexOf(new StringFinder(privelegedAssemblyName)) >= 0)
                            continue; // seen it before
                        internalAssemblies.Add(privelegedAssemblyName);


                        var builder = new CustomAttributeBuilder(
                            internalsVisibleToAttribType.GetConstructor(new[] {MapType(typeof (string))}),
        ...
        private static MethodBuilder EmitBoxedSerializer(TypeBuilder type, int i, Type valueType,
            SerializerPair[] methodPairs, TypeModel model, CompilerContext.IlVersion ilVersion, string assemblyName)
        {
            MethodInfo dedicated = methodPairs[i].Deserialize;
            var boxedSerializer = type.DefineMethod("_" + i, MethodAttributes.Static,
                CallingConventions.Standard,
                model.MapType(typeof (object)),
                new[] {model.MapType(typeof (object)), model.MapType(typeof (ProtoReader))});
            var ctx = new CompilerContext(boxedSerializer.GetILGenerator(), true, false, methodPairs, model, ilVersion,
                assemblyName, model.MapType(typeof (object)));
            ctx.LoadValue(ctx.InputValue);
            var @null = ctx.DefineLabel();
            ctx.BranchIfFalse(@null, true);

            var mappedValueType = valueType;
            ctx.LoadValue(ctx.InputValue);
            ...
            ctx.MarkLabel(@null);
            using (var typedVal = new Local(ctx, mappedValueType))
            {
        ...
        {
            var index = FindOrAddAuto(type, false, false, false);
            return index < 0 ? null : ((MetaType) _types[index]).GetEnumMap();
        }
    ...
    /// </summary>
                ...
                if (defaultType == null)
                {
...
#endif
                    {
                        ...
                        if (type.IsGenericType && type.GetGenericTypeDefinition() == MapType(typeof (IDictionary<,>))
                            &&
                            itemType ==
                            MapType(typeof (KeyValuePair<,>)).MakeGenericType(genArgs = type.GetGenericArguments()))
#endif
                        {
            ...
            ref bool requiresBclImport)
        {
            ...
            WireType wireType;
            var ser = ValueMember.TryGetCoreSerializer(this, dataFormat, effectiveType, out wireType, false,
                false,
                false, false);
            if (ser == null)
            {
    ...
    {
        private readonly string _ownerStackTrace;

        internal LockContentedEventArgs(string ownerStackTrace)
        {
            _ownerStackTrace = ownerStackTrace;
        }

...
namespace MyProto
{
    ...
    public abstract class Extensible : IExtensible
    {
        ...
        public static bool TryGetValue(TypeModel model, Type type, IExtensible instance, int tag, DataFormat format,
            bool allowDefinedTag, out object value)
        {
            value = null;
            var set = false;
            foreach (
            ...
            out TValue value)
        {
            ...
            var set = false;
            foreach (
                var val in ExtensibleUtil.GetExtendedValues<TValue>(instance, tag, format, true, allowDefinedTag))
            {
                // expecting at most one yield...
                // but don't break; need to read entire stream
                value = val;
